...
def multiple_streams_zoom(df,h5,lines,wftype,nrow,ncol,units,labs,filt,freq_min,freq_max,dt):

    """
    Build the multi-panel figure for events in counts and zoomin in ariound the P-arrival time
    lines : list of lines, lines number of the hdf5 File to plot
    wftype : str, waveform type 'ev_c', 'ev_gm' or 'noise' for events in counts, events in ground motion units or noise, respectively
    nrow : number of rows in figure
    ncol : numebr of columns in figure
    ...
    for i in range(nrow*ncol):
        ...
        sta = st[0].stats.station
        if wftype != 'noise':
                name = str(ev_id) + '.' + net + '.' + sta +'.'+str(P_date_time_obj.date()) + ' ' + str(polarity)
        else:
                name = str(ev_id)
